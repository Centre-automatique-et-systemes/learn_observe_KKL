...
if __name__ == "__main__":
    ...
    # Define data params
    wc_arr = np.linspace(0.1, 2., 5)
    # wc_arr = np.array([0.6])
    x_limits = np.array([[-2.7, 2.7], [-2.7, 2.7]])
    num_samples = wc_arr.shape[0] * 100000
    ...
    # path = "runs/SaturatedVanDerPol/Supervised_noise/T_star/exp_19/"  # TODO
    # import dill as pkl
    ...
    print(wc_arr)
    learner_T_star.save_random_traj(x_mesh, wc_arr, 1, verbose, (0, 20), 1e-2)

    # # Trajectories
    # std_array = [0.0, 0.25, 0.5]
    # wc_arr = np.array([2.8, 0.3, 0.6])
    # tsim = (0, 20)
    # dt = 1e-2
    # x_0 = torch.tensor([1.0, 1.0])
    #
    # learner_T_star.plot_rmse_error(x_0, wc_arr, verbose, tsim, dt, std=0.5)
    #
