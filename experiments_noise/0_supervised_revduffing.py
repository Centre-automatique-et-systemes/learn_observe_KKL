...
if __name__ == "__main__":
    ...
    # Define data params
    wc_arr = np.linspace(0.1, 2.0, 200)
    # D = 'diag'
    D = torch.tensor([[-1.5335, -2.5827, -0.1983],
                      [-0.6055, -2.6659, -0.3210],
                      [-0.6309, -0.1624, -2.9349]])
    ...
    if TRAIN:
        ...
        path = "runs/Reversed_Duffing_Oscillator/Supervised_noise/T_star/" \
               "exp200_DoptimAE05_wc01-2_rk41e-3_k10"
    ...
    wc_arr = np.array([0.03, 0.15, 1.])
    # wc_arr = np.array([0.1, 0.5, 2.0])
    tsim = (0, 50)
    dt = 1e-2
    ...
    learner_T_star.phase_portrait(
        init_state=x0, w_c_arr=wc_arr, verbose=verbose, tsim=(0, 8), dt=dt,
