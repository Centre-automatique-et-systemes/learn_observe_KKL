...
class LearnerNoise(Learner):
    ...
    def save_trj(self, init_state, w_c_arr, nb_trajs, verbose, tsim, dt, var=0.015):
        # Estimation over the test trajectories with T_star
        nb_trajs += w_c_arr.shape[0]
        traj_folder = os.path.join(self.results_folder, "Test_trajectories")
        tq, simulation = self.system.simulate(init_state, tsim, dt)
        noise = torch.normal(0, var, size=(simulation.shape[0], simulation.shape[1]))
    ...
    def plot_sensitiviy_wc(self, mesh, w_c_array, verbose, y_lim=[None, None]):
        ...
        plt.plot(
            w_c_array,
            errors[:, 0],
            label=r"$\frac{1}{N}||\frac{\partial{T^*}}{\partial{z}}(z)||$",
        )
        plt.plot(w_c_array, errors[:, 1], label=r"$\mathcal{H}_\infty$")
        plt.plot(
            w_c_array,
            errors[:, 2],
            label=r"$\frac{1}{N}||\frac{\partial{T^*}}{\partial{z}}(z)||\mathcal{H}_\infty$",
        )
